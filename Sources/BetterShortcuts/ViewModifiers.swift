...
extension View {
	...
	*/
	...
	public func onKeyboardShortcut(
		_ shortcut: BetterShortcuts.Name,
		perform: @escaping (BetterShortcuts.EventType) -> Void
	) -> some View {
		task {
			for await eventType in BetterShortcuts.events(for: shortcut) {
				perform(eventType)
			}
		}
	}
	...
	*/
	...
	public func onKeyboardShortcut(
		_ shortcut: BetterShortcuts.Name,
		type: BetterShortcuts.EventType,
		perform: @escaping () -> Void
	) -> some View {
		task {
			for await _ in BetterShortcuts.events(type, for: shortcut) {
				perform()
			}
		}
	}
...
extension View {
	...
	*/
	public func globalKeyboardShortcut(_ name: BetterShortcuts.Name) -> some View {
