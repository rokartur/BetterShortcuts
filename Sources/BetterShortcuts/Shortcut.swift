...
extension BetterShortcuts.Shortcut {
	/**
	System-defined keyboard shortcuts.
	*/
	static var system: [Self] {
		CarbonBetterShortcuts.system
	}
...
private let keyToSpecialKeyMapping: [BetterShortcuts.Key: SpecialKey] = [
	.return: .return,
	.delete: .delete,
...
extension BetterShortcuts.Shortcut: CustomStringConvertible {
	/**
	The string representation of the keyboard shortcut.

	```swift
	print(BetterShortcuts.Shortcut(.a, modifiers: [.command]))
	//=> "⌘A"
	```
	*/
			...
			let specialKey = keyToSpecialKeyMapping[key]
		{
			if let keyEquivalent = specialKey.swiftUIKeyEquivalent {
				if #available(macOS 12.0, *) {
					// We do `localization: .custom)` since the BetterShortcuts shortcuts are not localized.
